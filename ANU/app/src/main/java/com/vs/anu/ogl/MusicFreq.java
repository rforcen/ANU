...
import com.vs.anu.color.ColorScale;
...
public class MusicFreq {
	...
	private static String note_str[] = { "C", "C#", "D", "D#", "E", "F", "F#",
			"G", "G#", "A", "A#", "B" };
	...
	public static int Freq2Color(double freq) {
		...
		double f0 = NoteOct2Freq(0, oct), fz = NoteOct2Freq(0, oct + 1), ratio = (freq - f0)
				/ (fz - f0);

		// ratio between RED, VIOLET
		int col = ColorScale.interpolateColor(0xffff0000, 0xffff00ff,
				(float) ratio);
	...
	public static double FreqInOctave(double f, int o) {
		...
		if (f > fb)
			for (; f != 0 && f > fb && !(f >= fb && f <= ft); f /= 2)
				; // in octave
		else
			for (; f != 0 && f < ft && !(f >= fb && f <= ft); f *= 2)
				;
		return f;
	}
	...
	public static int Freq2NoteOctErr(double freq) {
		...
		// string
		if (NoteOct != null)
			NoteOct = (note_str[note])
					+ ((oct < 0) ? Integer.toString(oct) : "+"
							+ Integer.toString(oct));
		// error
